...
class Vulnerabilities(Base):
    ...
    detailed_info = Column(JSON, nullable=True)
...
def get_db_session():
    engine = create_engine('mysql+pymysql://root:Mega442001#@localhost/pro_secure', pool_pre_ping=True)
    Session = sessionmaker(bind=engine)
    return Session()
...
def detailed_info():
    ...
    if "detailed_info" not in columns:
        ...
        with engine.connect() as connection:
            connection.execute(text("ALTER TABLE vulnerabilities ADD COLUMN detailed_info JSON NULL;"))
            print("✅ Column 'detailed_info' added to vulnerabilities table!") 
